    ...
    (async () => {
      try {
        ...
        const merged: Ride[] = settled.map((r, idx) => {
          ...
          // 失败兜底：至少能显示 date/duration/issues count（你 adapter 里会算 duration）
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex items-center gap-3 p-4 bg-white border-b">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500">{loading ? "…" : `${rides.length} rides`}</p>
        </div>
      </div>
            ...
            return (
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-600">
                        {formatDate((ride as any).date ?? ride.startedAt?.toString?.() ?? "")}
                      </span>
                    </div>
                  ...
                  {/* Metrics: label + value, value aligned by grid rows */}
