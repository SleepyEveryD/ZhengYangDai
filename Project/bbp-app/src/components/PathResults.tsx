...
import { ArrowLeftIcon, StarIcon, ClockIcon, RulerIcon, TrendingUpIcon, LogInIcon } from 'lucide-react';
...
const mockRoutes: Route[] = [
  {
    ...
    segments: [
      { condition: 'excellent', distance: 3.5, description: 'Dedicated bike lane, smooth surface' },
      { condition: 'good', distance: 1.7, description: 'Shared lane, light traffic' },
    ],
    comments: [
      { user: 'Li Ming', date: '2025-11-05', content: 'Great road condition, recommended!', rating: 5 },
    ],
  },
  {
    id: '2',
    ...
    segments: [
      { condition: 'good', distance: 2.5, description: 'City road, moderate traffic' },
      { condition: 'fair', distance: 2.3, description: 'Minor pavement damage' },
    ],
    comments: [],
  },
  {
    ...
    segments: [
      { condition: 'excellent', distance: 4.0, description: 'Riverside bike path, beautiful scenery' },
      { condition: 'good', distance: 2.5, description: 'Park interior road' },
    ],
    comments: [
      { user: 'Wang Fang', date: '2025-11-03', content: 'Nice scenery, suitable for leisure cycling', rating: 4 },
    ],
  },
];

export default function PathResults({ navigateTo, user }: PathResultsProps) {
  const [selectedRouteId, setSelectedRouteId] = useState<string>('1');
  const [isAnimating, setIsAnimating] = useState(true);

  useEffect(() => {
    // Simulate route calculation animation
    ...
    <div className="h-screen flex flex-col bg-white">
      ...
      <div className="flex items-center gap-3 p-4 border-b bg-white z-10">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => navigateTo('pathPlanning')}
          className="h-10 w-10"
        >
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500">Found {mockRoutes.length} routes</p>
        </div>
      </div>
